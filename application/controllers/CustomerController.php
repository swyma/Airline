...
{
    protected $_numPerPage = 10; /* 每页10条 */
                        ...
                        if ($flightinformation == 0) {
                            ...
                            if ($fli_cangwei != null) {
                                ...
                                if ($permit) {
                                     ...
                                     */
                                    $fare_type = "select * from faretype where far_comcode='" .
                                     $com_code . "'";
                                    ...
                                    //获取用户的积分信息
                                    $cus_integral = "select cus_integral from customer where cus_id='" .
                                     $cus_id . "'";
                                    //实现业务:用户的积分与积分表的积分进行比较
                                    $cus_score = $adapter->fetchOne(
                                    $cus_integral);
                                    //积分进行比较,取得相应的折扣
                                    if ($cus_score >=
                                     $far_goldscore) {
                                        ...
                                        if ($cus_score >= $far_silscore) {
                                            $discount = $far_silddiscount;
                                        } else 
                                            if ($cus_score >= $far_broscore) {
                                                $discount = $far_brodiscount;
                                            }
                                    //折中折,生成最终的票价
                                    $fare = $standardMoney *
                                     $discount * $fli_discount; //最终用户价格
                                    /**
                                     * 业务:	生成座位号码
                                     * 涉及的表:refundrecord
                                     ...
                                     */
                                    $fli_allinfo = "select * from flightinformation where com_code='" .
                                     $com_code . "' and fli_no='" . $fli_no .
                                     "' and fli_everyday='" . $fli_everyday . "'";
                                    //取出各个字段的值
                                    $fli_info = $adapter->query(
                                    $fli_allinfo);
                                    ...
                                    'boo_number' => $cus_real_seat, 'boo_fare' => $fare, 
                                    'boo_time' => $time, 'flag_pay' => '1', 
                                    'flag_type' => '0', 'flag_pass' => '0');
                                    //该方法的api是这样的insert($array)
                                    $reuslt_insert = $db->insert(
                                    $data);
                                    if ($reuslt_insert > 0) {
                                        /**
                                         * 业务:更新用户的积分
                                        ...
                                        $data_update_sql = "update customer set cus_integral='" .
                                         ($fare + $cus_score) . "' where cus_id='" .
                                         $cus_id . "'";
                                        $result_update = $adapter->query(
                                        $data_update_sql);
                                        if ($result_update) {
                                            /**
                                             * 业务:更新相应舱位的座位号
                                            ...
                                            $reuslt_update_seat = $adapter->query(
                                            $seat_update);
                                            if ($reuslt_update_seat) {
                                                ...
                                                $show_sql = "select com_code,cus_id,boo_everyday,boo_no,boo_baddress,boo_aaddress,SUBSTRING(boo_btime,12,5) as boo_btime,SUBSTRING(boo_atime,12,5) as boo_atime,boo_berth,boo_number,boo_fare,boo_time from bookinformation where com_code='" .
                                                 $com_code . "' and cus_id='" . $cus_id .
                                                 "' and boo_everyday='" . $fli_everyday .
                                                 "' and boo_no='" . $fli_no .
                                                 "' and flag_pass=0";
                                                //执行sql语句
                                                $flightinformation = $adapter->query(
                                                $show_sql);
                                                $rows = $flightinformation->fetchAll();
                                                $ticketinformation = Zend_Json::encode(
                                                $rows);
                                                echo $tick_informations = substr(
                                                $ticketinformation, 0, 
                                                strlen($ticketinformation) - 2) .
                                                 (",\"ticket_no\":\"") . $ticket_no .
                                                 "\"}]";
                                                 //echo "订票成功!";
                                            }
