                                			...
                                			if($flight_occur>0){
                                				...
                                				if($flightinformation==0){
                                					...
                                					if($fli_cangwei!=null){
                                						...
                                						if($fli_cangwei=="公务舱"){
                                						...
                                						}else if($fli_cangwei=="经济舱"){
                                							//取出经济舱所有关于该舱位的所有信息
                                							/**
                                							 * 	1.直接取经济舱的价格fli_Cfare
                                							 */
                                							$GLOBALS['$fli_cangweiinfo']="select fli_Cnumber as fli_number,fli_Cfare as fli_fare,fli_discount as fli_discount from flightinformation where flag=1  and com_code='".$com_code."' and fli_no='".$fli_no."'and fli_everyday='".$fli_everyday."'";
                                							$refund_info_sql="select count(r.boo_number) from refundrecord r ,bookinformation b where r.boo_autoid=b.boo_autoid and b.com_code='".$com_code."' and b.boo_no='".$fli_no."' and b.boo_berth='".$fli_cangwei."' and b.boo_everyday='".$fli_everyday."'";
                                							$refund_info = $adapter->fetchOne($refund_info_sql);
                                							if($refund_info>0){
                                								//如果退票表有相应航班相应舱位信息的话
                                								 ...
                                								 * $fli_cangweicount:舱位人数的多少
                                								 */
                                								$fli_cangwei_sql="select count(*) from flightinformation where flag=1 and fli_Cnumber>0 and com_code='".$com_code."' and fli_no='".$fli_no."' and fli_everyday='".$fli_everyday."'";
                                								$fli_cangweicount=$adapter->fetchOne($fli_cangwei_sql);
                                								if($fli_cangweicount>0){
                                									//航班的相应舱位有座位提供
                                									$permit=true;
                                							...
                                							$GLOBALS['$fli_cangweiinfo']="select fli_Fnumber as fli_number,fli_Cfare as fli_fare,fli_discount from flightinformation where flag=1 and com_code='".$com_code."' and fli_no='".$fli_no."' and fli_everyday='".$fli_everyday."'";
                                							$refund_info_sql="select count(r.boo_number) from refundrecord r ,bookinformation b where r.boo_autoid=b.boo_autoid and b.com_code='".$com_code."' and b.boo_no='".$fli_no."' and b.boo_berth='".$fli_cangwei."' and b.boo_everyday='".$fli_everyday."'";
                                							$refund_info = $adapter->fetchOne($refund_info_sql);
                                							if($refund_info>0){
                                								//如果退票表有相应航班相应舱位信息的话
                                						...
                                						//已经允许用户订票
                                						if($permit){
                                							$moneyandseat = $adapter->query($GLOBALS['$fli_cangweiinfo']);
                                							...
                                							if($cus_score>=$far_goldscore){
                                								$discount=$far_golddiscount;
                                							}else if($cus_score>=$far_silscore){
                                								$discount=$far_silddiscount;
                                							}else if($cus_score>=$far_broscore){
                                								$discount=$far_brodiscount;
                                							}
                                							 
                                							//折中折,生成最终的票价
                                							$fare=$standardMoney*$discount*$fli_discount;				//最终用户价格
                                							...
                                							$seat="select r.boo_number from refundrecord r ,bookinformation b where r.boo_autoid=b.boo_autoid and b.flag_pass=1 and b.com_code='".$com_code."' and b.boo_no='".$fli_no."' and b.boo_everyday='".$fli_everyday."'";
                                							//取得第一条记录中的字段
                                							//取到第一个座位号
                                							$seat_virtual_no = $adapter->fetchOne($seat);
                                							...
                                							if($seat_virtual_no!=null){
                                								...
                                								//得到相应航班的座位号
                                								if($fli_cangwei=="经济舱"){
                                									$seat_no_sql="select fli_Cnumber as seat_no from flightinformation where com_code='".$com_code."' and fli_no='".$fli_no."' and flag=1";
                                									$seat_no = $adapter->fetchOne($seat_no_sql);
                                								}else if($fli_cangwei=="头等舱"){
                                									$seat_no_sql="select fli_Fnumber as seat_no from flightinformation where com_code='".$com_code."' and fli_no='".$fli_no."' and flag=1";
                                									$seat_no = $adapter->fetchOne($seat_no_sql);
                                								}else if($fli_cangwei=="公务舱"){
                                									$seat_no_sql="select fli_Ynumber as seat_no from flightinformation where com_code='".$com_code."' and fli_no='".$fli_no."' and flag=1";
                                									$seat_no = $adapter->fetchOne($seat_no_sql);
                                								}
                                							}
                                							//得到用户最后真正的座位号
                                							...
                                							//相关数据数组化,为insert服务
                                							$data = array('com_code' => $com_code, 'cus_id' => $cus_id,'boo_everyday'=>$fli_everyday,'boo_no'=>$fli_no,'boo_baddress'=>$fli_baddress,
                                		                					  'boo_aaddress'=>$fli_aaddress,'boo_atime'=>$fli_atime,'boo_btime'=>$fli_btime,'boo_berth'=>$fli_cangwei,'boo_number'=>$cus_real_seat,
                                		                					  'boo_fare'=>$fare,'boo_time'=>$time,'flag_pay'=>'1','flag_type'=>'0','flag_pass'=>'0');
                                							...
                                							if($reuslt_insert>0){
                                								...
                                								$data_update_sql="update customer set cus_integral='".($fare+$cus_score)."' where cus_id='".$cus_id."'";
                                								...
                                								if($result_update){
                                									...
                                									if($fli_cangwei=="经济舱"){
                                										if($seat_virtual_no!=null){
                                											//删除退票表相应的信息
                                											$seat_update="delete from refundrecord where boo_autoid=(select boo_autoid from bookinformation where flag_pass=1 and com_code='".$com_code."' and boo_no='".$fli_no."' and boo_number='".$seat_virtual_no."' and boo_everyday='".$fli_everyday."')";
                                											//$reuslt=$adapter->query($seat_update);	//删除完毕
                                										}else{
                                											$seat_update="update flightinformation set fli_Cnumber='".($seat_no-1)."' where fli_no='".$fli_no."' and com_code='".$com_code."' and fli_everyday='".$fli_everyday."'";
                                										}
                                									}else if($fli_cangwei=="头等舱"){
                                										if($seat_virtual_no!=null){
                                											$seat_update="delete from refundrecord where boo_autoid=(select boo_autoid from bookinformation where flag_pass=1 and com_code='".$com_code."' and boo_no='".$fli_no."' and boo_number='".$seat_virtual_no."' and boo_everyday='".$fli_everyday."')";
                                											...
                                											$seat_update="update flightinformation set fli_Fnumber='".($seat_no-1)."' where fli_no='".$fli_no."' and com_code='".$com_code."'and fli_everyday='".$fli_everyday."'";
                                										}
                                									}else if($fli_cangwei=="公务舱"){
                                										if($seat_virtual_no!=null){
                                											$seat_update="delete from refundrecord where boo_autoid=(select boo_autoid from bookinformation where flag_pass=1 and com_code='".$com_code."' and boo_no='".$fli_no."' and boo_number='".$seat_virtual_no."' and boo_everyday='".$fli_everyday."')";
                                											...
                                											$seat_update="update flightinformation set fli_Ynumber='".($seat_no-1)."' where fli_no='".$fli_no."' and com_code='".$com_code."'and fli_everyday='".$fli_everyday."'";
                                										}
                                									}
                                									...
                                									if($reuslt_update_seat){
                                										...
                                										$ticket_no=time().$cus_id.$cus_real_seat;
                                										//将信息传递给前台,前台接受并显示相应的信息
                                										$show_sql = "select com_code,cus_id,boo_everyday,boo_no,boo_baddress,boo_aaddress,SUBSTRING(boo_btime,12,5) as boo_btime,SUBSTRING(boo_atime,12,5) as boo_atime,boo_berth,boo_number,boo_fare,boo_time from bookinformation where com_code='".$com_code."' and cus_id='".$cus_id."' and boo_everyday='".$fli_everyday."' and boo_no='".$fli_no."' and flag_pass=0";
                                										//执行sql语句
    ...
    public function customerloginAction()
    {
        $this->_helper->layout->disableLayout();
                                        ...
                                        $userNamespace->__unset();//注销session*/
                                                ...
                                                $adapter = Zend_Registry::get('db');
                                                //查询登录会员的信息
                                               $sqlstr1="select count(*) from customer where  cus_account='".$cus_account."' and
